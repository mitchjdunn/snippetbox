...
// Insert this will insert a new snippet int othe database
func (m *SnippetModel) Insert(title string, content string, expires int) (int, error) {

	// Write the SQL statement we want to execute. I've split it over two lines
	// for readability, reason for back ticks instead of quotes
	stmt := `INSERT INTO snippets (title, content, created, expires)
    		VALUES(?, ?, UTC_TIMESTAMP(), DATE_ADD(UTC_TIMESTAMP(), INTERVAL ? DAY))`

	// use the Exec() method on the embedded connection pool to execute the
	// statement. the first parameters is the SQL statement, followed by the
	// values for the placeholder parameters: title, content, expiry
	...
	// new inserted record in the snippets table.
...
// Latest This will return the 10 most recent snippets
